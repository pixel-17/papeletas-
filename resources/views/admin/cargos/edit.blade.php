<x-app-layout>
    ...
    <form method="POST" action="{{ route('cargos.update', $cargo) }}" class="bg-white rounded-lg shadow-sm p-5 space-y-4 max-w-lg">
        @csrf @method('PUT')
        <div>
        ...
        <div>
            ...
            <input type="text" name="nombre" required value="{{ old('nombre', $cargo->nombre) }}" class="w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
        </div>
        <div class="flex items-center gap-2">
            <input type="hidden" name="estado" value="0">
            <input type="checkbox" name="estado" value="1" id="estado" class="rounded" @checked(old('estado', $cargo->estado))>
